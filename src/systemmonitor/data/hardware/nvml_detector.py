...
class NVMLDetector(GPUDetector):
    ...
    def _init_nvml(self):
        """Initialize NVML with robust error handling and modern library support"""
        try:
            # We try to use nvidia-ml-py which still exposes the 'pynvml' module name
            # but we use a more robust import check
            import pynvml
            try:
                pynvml.nvmlInit()
                self._nvml = pynvml
                ...
                self.logger.info(f"NVML available but could not initialize (likely no NVIDIA GPU or driver): {e}")
                self._nvml_available = False
        except (ImportError, ModuleNotFoundError):
            # Try alternative import for some versions of the library
            try:
                ...
                self._nvml_available = True
                self._device_count = self._nvml.nvmlDeviceGetCount()
                self.logger.info("NVML initialized via nvidia-smi module")
            except:
                self.logger.debug("NVML module not found")
                self._nvml_available = False
        except Exception as e:
            self.logger.debug(f"NVML init failed: {e}")
            self._nvml_available = False
