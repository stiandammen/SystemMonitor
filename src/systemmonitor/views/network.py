...
from PyQt6.QtWidgets import (
    ...
    QProgressBar, QPushButton, QSizePolicy, QGridLayout
)
from PyQt6.QtCore import Qt, QTimer, QPointF, QRectF
from PyQt6.QtGui import (
    QFont, QColor, QPainter, QPen, QLinearGradient, QBrush,
...
from systemmonitor.typing import Dict, List, Optional
...
class _IfaceRow(QFrame, ScaleMixin):
    ...
    def __init__(self, name: str, is_up: bool, speed: int, ip: str, parent=None):
        ...
        # speed badge
        speed_str = f"{speed} Mbps" if speed > 0 else ("Up" if is_up else "Down")
...
class NetworkView(QWidget, ScaleMixin):
    ...
    def _build_kpi_row(self, parent):
        ...
        self._kpi_down  = KpiCard("Download",    "mdi.arrow-down-bold",   c.ACCENT_CYAN,   "Mbps")
        self._kpi_up    = KpiCard("Upload",       "mdi.arrow-up-bold",     c.ACCENT_PURPLE, "Mbps")
    ...
    def _build_traffic_card(self, parent):
        card = Card(title="Network Traffic", icon="ph.chart-line")
    ...
    def _build_middle_row(self, parent):
        ...
        conn_card = Card(title="Active Connections", icon="ph.link")
        ...
        proto_card = Card(title="Protocol Distribution", icon="ph.chart-pie")
        ...
        # Interface status
        iface_card = Card(title="Network Interfaces", icon="ph.globe")
    ...
    def _build_radar_row(self, parent):
        ...
        # Radar / topology
        radar_card = Card(title="Network Topology", icon="ph.broadcast")
    ...
    def _start(self):
        ...
        QTimer.singleShot(600, self._refresh_connections)
    ...
    def _refresh_connections(self):
        ...
        self._kpi_conns.set_value(str(len(rows)), "TCP + UDP")
        ...
        for name, stat in list(stats.items())[:8]:
            ...
            for addr in addrs.get(name, []):
                if addr.family == socket.AF_INET:
                    ip = addr.address
                    break
            row = _IfaceRow(name, stat.isup, stat.speed, ip)
            self._iface_list.addWidget(row)
            if stat.isup:
                up_count += 1

        self._kpi_iface.set_value(str(up_count), f"of {len(stats)} interfaces")
