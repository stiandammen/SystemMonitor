...
from PyQt5.QtWidgets import (
    ...
    QLabel, QFrame, QProgressBar, QScrollArea, QSizePolicy, QMenu
)
...
class MemoryDonutChart(QWidget):
    ...
    def paintEvent(self, event):
        ...
        painter.setPen(QPen(QColor(colors.ACCENT_GREEN), 12, Qt.SolidLine, Qt.RoundCap))
        ...
        painter.setPen(QPen(QColor(colors.ACCENT_PURPLE), 12, Qt.SolidLine, Qt.RoundCap))
        painter.drawArc(int(arc_rect), int(arc_rect), int(2 * radius), int(2 * radius),
        ...
        painter.setPen(QPen(QColor(colors.ACCENT_BLUE), 12, Qt.SolidLine, Qt.RoundCap))
        painter.drawArc(int(arc_rect), int(arc_rect), int(2 * radius), int(2 * radius),
...
class MemoryUsageGraph(QWidget):
    ...
    def paintEvent(self, event):
        ...
        if len(self._history) > 1:
            ...
            painter.setPen(QPen(glow_color, 6, Qt.SolidLine))
            for i in range(len(points) - 1):
                painter.drawLine(int(points[i][0]), int(points[i][1]),
                               int(points[i + 1][0]), int(points[i + 1][1]))
        ...
        if self._history:
            ...
            painter.setBrush(QColor(colors.BG_SECONDARY))
            painter.setPen(Qt.NoPen)
            painter.drawRoundedRect(int(badge_x), int(badge_y), 55, 22, 4)
...
class ProcessRow(QFrame):
    ...
    def _setup_ui(self):
        ...
        self._rank_lbl.setFont(QFont("Segoe UI", 9, QFont.Bold))
        self._rank_lbl.setStyleSheet(f"color: {colors.TEXT_MUTED};")
        ...
        self._mem_lbl.setStyleSheet(f"color: {colors.ACCENT_CYAN};")
        ...
        self._pct_lbl.setStyleSheet(f"color: {colors.TEXT_SECONDARY};")
    ...
    def update_values(self, name: str, memory_mb: float = 0, percent: float = 0, rank: int = 0, pid: int = None):
        self._rank = rank
        self._pid = pid
...
class MemoryStatCard(QFrame):
    ...
    def __init__(self, label: str = "", value: str = "--", color: str = None, parent=None):
        ...
        self._value_lbl.setFont(QFont("Segoe UI", 14, QFont.Bold))
...
class MemoryView(QWidget):
    ...
    def _setup_ui(self):
        ...
        self._status_dot.setStyleSheet(f"color: {colors.ACCENT_GREEN}; font-size: 12px;")
        ...
        self._usage_indicator.setFont(QFont("Segoe UI", 16, QFont.Bold))
        self._usage_indicator.setStyleSheet(f"color: {colors.ACCENT_GREEN};")
        ...
        donut_title.setFont(QFont("Segoe UI", 9, QFont.Bold))
        donut_title.setStyleSheet(f"color: {colors.TEXT_SECONDARY};")
        donut_layout.addWidget(donut_title)

        self._donut_chart = MemoryDonutChart()
        donut_layout.addWidget(self._donut_chart, alignment=Qt.AlignCenter)
        ...
        graph_title.setFont(QFont("Segoe UI", 9, QFont.Bold))
        graph_title.setStyleSheet(f"color: {colors.TEXT_SECONDARY};")
        ...
        process_title = QLabel("Top 10 Processes by Memory")
        process_title.setFont(QFont("Segoe UI", 9, QFont.Bold))
        process_title.setStyleSheet(f"color: {colors.TEXT_SECONDARY};")
        ...
        pressure_title.setFont(QFont("Segoe UI", 9, QFont.Bold))
        pressure_title.setStyleSheet(f"color: {colors.TEXT_SECONDARY};")
        pressure_title_row.addWidget(pressure_title)
        pressure_title_row.addStretch()

        self._pressure_status = QLabel("Normal")
        self._pressure_status.setFont(QFont("Segoe UI", 9, QFont.Bold))
        self._pressure_status.setStyleSheet(f"color: {colors.ACCENT_GREEN}; background-color: {colors.BG_SECONDARY}; padding: 3px 10px; border-radius: 8px;")
    ...
    def _update_stats(self, mem_data):
        ...
        if used_pct > 90:
            self._usage_indicator.setStyleSheet(f"color: {colors.ACCENT_RED}; font-size: 16px; font-weight: bold;")
        elif used_pct > 70:
            self._usage_indicator.setStyleSheet(f"color: {colors.ACCENT_ORANGE}; font-size: 16px; font-weight: bold;")
        elif used_pct > 40:
            self._usage_indicator.setStyleSheet(f"color: {colors.ACCENT_YELLOW}; font-size: 16px; font-weight: bold;")
        else:
            self._usage_indicator.setStyleSheet(f"color: {colors.ACCENT_GREEN}; font-size: 16px; font-weight: bold;")
