...
from widgets.settings_row import ToggleWidget
...
class SettingsView(QWidget, ScaleMixin):
    ...
    def _build_general_section(self, parent: QVBoxLayout):
        card = Card(title="General", icon="⚙️")
    ...
    def _build_appearance_section(self, parent: QVBoxLayout):
        card = Card(title="Appearance", icon="🎨")
    ...
    def _build_maintenance_section(self, parent: QVBoxLayout):
        card = Card(title="Maintenance", icon="🔧")

        reset_btn = self._btn("Reset all settings to default", danger=True)
